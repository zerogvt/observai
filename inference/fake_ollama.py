...
service (and the whole gateway -> inference chain) before pulling llama3.2.
...
def tags():
    ...
    return jsonify(models=[{"name": "llama3.2"}])
...
def chat():
    ...
    if "empty" in user_text.lower():
        ...
        content = f"[fake llama3.2 reply to {len(user_text)} chars of input]"
